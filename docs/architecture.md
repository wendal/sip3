...
| WebRTC gateway | `backend/src/sip/webrtc_gateway.rs` | Browser ICE/DTLS-SRTP to plain SIP RTP bridging for `/phone` audio/video interop paths (including browser↔Linphone video). |
...
| Frontend | `frontend/src` | Vue 3 + Element Plus admin dashboard and `/phone` browser softphone for audio/video, including browser↔Linphone video calls. |
